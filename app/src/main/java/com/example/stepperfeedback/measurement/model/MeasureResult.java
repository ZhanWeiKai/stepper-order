package com.example.stepperfeedback.measurement.model;
...
public class MeasureResult {
    ...
    /** 动态阈值 X (投影过滤) */
    public final int dynamicThresholdX;

    /** 动态阈值 Y (投影过滤) */
    public final int dynamicThresholdY;

    /** Mask阈值 (mm) - 初始50mm，不稳定时+5mm */
    public final float maskThreshold;

    /** Mask阈值是否已锁定 */
    public final boolean maskThresholdLocked;
    ...
    public MeasureResult(float width, float length, float height,
        ...
        this(width, length, height, rawPixelCount, validPixelCount,
                maxDepthDiff, objectSurfaceDepth, xMin, xMax, yMin, yMax,
                isStable, stableCount, message, 0, 0, 50f, false);
    }
    ...
    public MeasureResult(float width, float length, float height,
        ...
        this(width, length, height, rawPixelCount, validPixelCount,
                maxDepthDiff, objectSurfaceDepth, xMin, xMax, yMin, yMax,
                isStable, stableCount, message, dynamicThresholdX, dynamicThresholdY, 50f, false);
    }
    ...
    public MeasureResult(float width, float length, float height,
                         ...
                         boolean isStable, int stableCount, String message,
                         int dynamicThresholdX, int dynamicThresholdY,
                         float maskThreshold, boolean maskThresholdLocked) {
        this.width = width;
        this.length = length;
        this.height = height;
        ...
        this.maskThresholdLocked = maskThresholdLocked;
    }
    ...
    public String getFormattedResult() {
        ...
        sb.append(String.format("状态: %s (%d/10帧)\n", isStable ? "稳定" : "不稳定", stableCount));
        sb.append(String.format("Mask阈值: %.0fmm %s\n", maskThreshold, maskThresholdLocked ? "(已锁定)" : ""));
        sb.append(String.format("有效像素: %d (原始: %d)\n", validPixelCount, rawPixelCount));
        sb.append(String.format("投影阈值: X=%d, Y=%d\n", dynamicThresholdX, dynamicThresholdY));
